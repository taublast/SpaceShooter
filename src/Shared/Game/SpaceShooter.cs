...
public partial class SpaceShooter
{
    ...
    public override void OnKeyUp(InputKey key)
    {
        ...
        if (gameKey == GameKey.Fire && (State == GameState.Ready || State == GameState.Ended))
        {
            StartNewGame();
            return;
        }
    ...
    /// so we dont want bindings to update the score too often.
    ...
    /// <summary>
    /// This could be changed from loop and from individual sprite animation thread
    /// </summary>
